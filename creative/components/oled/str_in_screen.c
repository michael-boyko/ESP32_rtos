...
void str_in_screen(char *str, uint8_t **screen, _Bool is_big) {   //big
    uint8_t *symbol = NULL;
    int start_point = 0;

    if (is_big == 1) {
        start_point = (128 - strlen(str) * 12) / 2;
        ...
        start_point = (128 - strlen(str) * 6) / 2;
        for (int j = 0; str[j] != '\0'; ++j) {
            symbol = get_symbol_code(str[j]);
            for (int i = 0; i < 6; ++i) {
                screen[5][start_point + (j * 6) + i] = symbol[i];
            }
        free(symbol);
        }
    }
